...
import { apiRequest, handleFileUpload } from "../utils/apiUtils";
...
const CompanyForm = ({ open, setOpen }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setIsLoading(false);
      console.log("Successfully updated company profile", res);
    ...
    <div className="container mx-auto p-5">
      <div className="">
        <div className="w-full flex flex-col md:flex-row gap-3 justify-between">
          ...
          {user?.accountType === "company" && params.id === user?._id && (
      ...
      <div className="mt-5">
        <p className="text-gray-600 text-xl font-semibold">Jobs Posted</p>

        <div class="w-full flex flex-wrap p-4 gap-4">
          {recentPost?.jobPosts?.length > 0 ? (
            recentPost?.jobPosts?.map((job, index) => {
              const data = {
                company: {
                  name: recentPost.name,
                  email: recentPost.email,
                  profileUrl: recentPost.profileUrl,
                },
                ...job,
              };
            ...
            <p className="text-gray-600 text-xl font-semibold text-center w-100">
              No Job Found
            </p>
          )}
        </div>
