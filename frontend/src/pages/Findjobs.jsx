...
const FindJobs = () => {
  ...
  const FindJobs = async () => {
    ...
    const newURL = updateURL({
      ...
      navigate,
      location,
      jType: filterJobTypes.length > 0 ? filterJobTypes : "",
      exp: filterExp.length > 0 ? filterExp : "",
    });
    console.log(newURL);
    try {
  ...
  const filterExperience =  (value) => {
    ...
    if (filterExp?.includes(value)) {
      setFilterExp(filterExp.filter((el) => el !== value));
    } else {
  ...
  return (
    <div>
      <Header
        title="Land Your Perfect Job Effortlessly"
        type="home"
        handleClick={() => {}}
            ...
            {data.map((job, index) => {
              const newJob = {
                company: {
                  ...
                  profileUrl: data?.company?.profileUrl,
                },
                ...job,
              };
